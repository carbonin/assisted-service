...
func main() {
	...
	installConfigBuilder := installcfg.NewInstallConfigBuilder(log.WithField("pkg", "installcfg"))
	...
	objectHandler := createStorageClient(Options.DeployTarget, Options.Storage, &Options.S3Config,
		Options.JobConfig.WorkDir, log, versionHandler, isoEditorFactory, metricsManager, staticNetworkConfig)
...
func createStorageClient(deployTarget string, storage string, s3cfg *s3wrapper.Config, fsWorkDir string,
	...
	metricsAPI metrics.API, staticNetworkConfig staticnetworkconfig.StaticNetworkConfig) s3wrapper.API {
	...
	if storage != "" {
		...
		case storage_filesystem:
			storageClient = s3wrapper.NewFSClient(fsWorkDir, log, versionsHandler, isoEditorFactory, metricsAPI, staticNetworkConfig)
			if storageClient == nil {
