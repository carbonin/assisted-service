...
var _ = Describe("tangConnectivitycheckcmd", func() {
	...
	It("get_step: Tang EnableOnAll", func() {
		...
		Expect(stepReply[0].Args[len(stepReply[0].Args)-1]).Should(Equal("[{\"URL\":\"http://tang.example.com:7500\",\"Thumbprint\":\"\"}]"))
	...
	It("get_step: day2 host", func() {
		clusterID := strfmt.UUID(uuid.New().String())

		day2Host := hostutil.GenerateTestHostAddedToCluster(
			strfmt.UUID(uuid.New().String()), infraEnvID, clusterID, models.HostStatusInsufficient)
		Expect(db.Create(&day2Host).Error).ShouldNot(HaveOccurred())

		stepReply, stepErr = tangConnectivityCheckCmd.GetSteps(ctx, &day2Host)
		Expect(len(stepReply)).Should(Equal(0))
		Expect(stepErr).ShouldNot(HaveOccurred())
	})
