...
func (r *AgentServiceConfigReconciler) Reconcile(origCtx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !r.IsOpenShift {
		var err error
		supportsCertManager, err = serverSupportsCertManager(ctx, r.Client)
		if err != nil {
	...
	valid, err := r.validate(ctx, log, asc, supportsCertManager)
	if err != nil {
		return ctrl.Result{Requeue: true}, err
	}
	if !valid {
	...
	if err := r.cleanHTTPRoute(ctx, log, asc); err != nil {
		return ctrl.Result{Requeue: true}, err
	}
...
func (r *AgentServiceConfigReconciler) getComponents(spec *aiv1beta1.AgentServiceConfigSpec, isOpenshift bool, annotations map[string]string) []component {
	...
	if imageServiceEnabled {
		components = append(components,
			component{"ImageServiceService", aiv1beta1.ReasonImageHandlerServiceFailure, r.newImageServiceService},
			component{"ImageServiceServiceAccount", aiv1beta1.ReasonImageHandlerServiceAccountFailure, r.newImageServiceServiceAccount},
			component{"ImageServiceRoute", aiv1beta1.ReasonImageHandlerRouteFailure, r.newImageServiceRoute},
		)
	}

	if isOpenshift {
		components = append(components,
			component{"ServiceMonitor", aiv1beta1.ReasonAgentServiceMonitorFailure, r.newServiceMonitor},
			component{"IngressCertConfigMap", aiv1beta1.ReasonIngressCertFailure, r.newIngressCertCM},
			component{"ClusterTrustedCAConfigMap", aiv1beta1.ReasonConfigFailure, r.newClusterTrustedCACM},
			component{"AssistedTrustedCAConfigMap", aiv1beta1.ReasonConfigFailure, r.newAssistedTrustedCACM},
		)

		if imageServiceEnabled {
			components = append(components,
				// this is only for mounting in the https certs
				component{"ImageServiceConfigMap", aiv1beta1.ReasonConfigFailure, r.newImageServiceConfigMap},
			)
		}
		...
		if exposeIPXEHTTPRoute(spec) {
			components = append(components,
				component{"AgentIPXERoute", aiv1beta1.ReasonAgentRouteFailure, r.newAgentIPXERoute},
			)
			if imageServiceEnabled {
				components = append(components,
					component{"ImageServiceIPXERoute", aiv1beta1.ReasonImageHandlerRouteFailure, r.newImageServiceIPXERoute},
				)
			}
		}
	} else {
		components = append(components, r.certManagerComponents()...)
	}
	// needs to be created after all routes to pull the hostnames into the configmap
	components = append(components,
		component{"AssistedServiceConfigMap", aiv1beta1.ReasonConfigFailure, r.newAssistedCM})
	components = append(components,
		// needs to be created after all of the configmaps to calculate the config hash and ensure the correct data exists
		component{"AssistedServiceDeployment", aiv1beta1.ReasonDeploymentFailure, r.newAssistedServiceDeployment})
	return components
}
...
func (r *AgentServiceConfigReconciler) ensureFinalizers(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig, finalizerName string) error {
	if asc.GetDeletionTimestamp().IsZero() {
		...
		if err := r.Client.Get(ctx, client.ObjectKeyFromObject(statefulSet), statefulSet); err != nil && !errors.IsNotFound(err) {
			log.WithError(err).Error("failed to get image service stateful set for cleanup")
			return err
		}
		if err := r.cleanupImageServiceFinalizer(ctx, asc, statefulSet); err != nil {
...
func (r *AgentServiceConfigReconciler) monitorOperands(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig) (string, error) {
	...
	for _, deployName := range deploys {
		...
		if err := r.Client.Get(ctx, types.NamespacedName{Name: deployName, Namespace: r.Namespace}, deployment); err != nil {
			return "", err
		}
	...
	return r.monitorImageServiceStatefulSet(ctx, log, asc)
}
...
func (r *AgentServiceConfigReconciler) monitorImageServiceStatefulSet(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig) (string, error) {
	...
	if err := r.Client.Get(ctx, types.NamespacedName{Name: imageServiceName, Namespace: r.Namespace}, ss); err != nil {
		return "", err
	}
		...
		sm.Spec.Endpoints = []monitoringv1.Endpoint{{
			...
			TLSConfig: &monitoringv1.TLSConfig{
				...
				SafeTLSConfig: monitoringv1.SafeTLSConfig{
					ServerName: fmt.Sprintf("%s.%s.svc", serviceName, r.Namespace),
				},
			},
		}}
...
func (r *AgentServiceConfigReconciler) newAssistedTrustedCACM(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig) (client.Object, controllerutil.MutateFn, error) {
	...
	if err := r.Client.Get(ctx, types.NamespacedName{Name: clusterCAConfigMapName, Namespace: r.Namespace}, clusterTrustedCACM); err != nil {
...
func (r *AgentServiceConfigReconciler) urlForRoute(ctx context.Context, asc *aiv1beta1.AgentServiceConfig, routeName string) (string, error) {
	...
	if r.IsOpenShift {
		route := &routev1.Route{}
		err := r.Client.Get(ctx, types.NamespacedName{Name: routeName, Namespace: r.Namespace}, route)
...
func (r *AgentServiceConfigReconciler) newAssistedCM(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig) (client.Object, controllerutil.MutateFn, error) {
	...
	mutateFn := func() error {
		...
		cm.Data = map[string]string{
			...
			"PUBLIC_CONTAINER_REGISTRIES": r.unauthenticatedRegistries(ctx, asc),
		...
		r.getDeploymentData(ctx, cm, asc)
...
func (r *AgentServiceConfigReconciler) getDeploymentData(ctx context.Context, cm *corev1.ConfigMap, asc *aiv1beta1.AgentServiceConfig) {
	...
	if err := r.Client.Get(ctx, types.NamespacedName{Name: acmDeployName, Namespace: acmDeployNamespace}, deploy); err == nil {
	...
	if err := r.Client.Get(ctx, types.NamespacedName{Name: mceDeployName, Namespace: mceDeployNamespace}, deploy); err == nil {
...
func (r *AgentServiceConfigReconciler) newImageServiceStatefulSet(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig) (*appsv1.StatefulSet, controllerutil.MutateFn) {
	...
	mutateFn := func() error {
		...
		r.injectImagePullSecretsWhenNonOCP(ctx, &statefulSet.Spec.Template)
...
func (r *AgentServiceConfigReconciler) cleanupImageServiceFinalizer(ctx context.Context, asc *aiv1beta1.AgentServiceConfig, statefulSet *appsv1.StatefulSet) error {
	...
	if err := r.Client.List(ctx, pvcList, client.MatchingLabels{"app": imageServiceName}); err != nil {
		return err
	}
...
func (r *AgentServiceConfigReconciler) reconcileImageServiceStatefulSet(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig) error {
	...
	statefulSet, mutateFn := r.newImageServiceStatefulSet(ctx, log, asc)
...
func (r *AgentServiceConfigReconciler) newAssistedServiceDeployment(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig) (client.Object, controllerutil.MutateFn, error) {
	...
	assistedConfigHash, err := r.getCMHash(ctx, asc, types.NamespacedName{Name: serviceName, Namespace: r.Namespace})
	if err != nil {
		return nil, nil, err
	}
	...
	if r.IsOpenShift {
		...
		cm := &corev1.ConfigMap{}
		namespacedName := types.NamespacedName{Name: assistedCAConfigMapName, Namespace: r.Namespace}
	...
	mutateFn := func() error {
		...
		r.injectImagePullSecretsWhenNonOCP(ctx, &deployment.Spec.Template)
...
func (r *AgentServiceConfigReconciler) injectImagePullSecretsWhenNonOCP(ctx context.Context, podTemplateSpec *corev1.PodTemplateSpec) {
	if !r.IsOpenShift {
		if r.PodIntrospector == nil {
			return
		}
		if imagePullSecrets := r.PodIntrospector.GetImagePullSecrets(ctx); len(imagePullSecrets) > 0 {
...
func (r *AgentServiceConfigReconciler) newWebHookAPIService(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig) (client.Object, controllerutil.MutateFn, error) {
	...
	mutateFn := func() error {
		...
		if r.IsOpenShift {
			...
			setAnnotation(&as.ObjectMeta, certManagerCAInjectionAnnotation, fmt.Sprintf("%s/%s", r.Namespace, webhookServiceName))
		}
		baseApiServiceSpec(as, r.Namespace)
		return nil
	}
...
func (r *AgentServiceConfigReconciler) validateOSImageCACertRef(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig) (bool, error) {
	if asc.Spec.OSImageCACertRef != nil && asc.Spec.OSImageCACertRef.Name != "" {
		...
		if err != nil {
			return false, r.registerCACertFailureCondition(ctx, log, err, asc)
		}
...
func (r *AgentServiceConfigReconciler) validateImmutableAnnotations(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig) (bool, error) {
	...
	for _, annotation := range immutableAnnotations {
		...
		if !initialExists && currentExists {
			...
			return false, r.registerImmutableAnnotationFailureCondition(ctx, log, err, asc)
		}
...
func (r *AgentServiceConfigReconciler) validate(ctx context.Context, log logrus.FieldLogger, asc *aiv1beta1.AgentServiceConfig, supportsCertManager bool) (bool, error) {
	...
	if valid, err := r.validateImmutableAnnotations(ctx, log, asc); !valid {
		return false, err
	}
	...
	// reconciliation.
	warnings, failures, err := r.validateStorage(ctx, log, asc)
	if err != nil {
