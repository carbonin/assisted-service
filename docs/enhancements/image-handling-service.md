...
1. Image service receives a request to download an image
  - This request contains, as query parameters, the assisted service auth token, the image type (minimal/full), and image version
...
Today the ignition is created and uploded separately to S3 by the assisted
...
- How does traffic get to the new service?
  - [Path based routes?](https://docs.openshift.com/container-platform/4.7/networking/routes/route-configuration.html#nw-path-based-routes_route-configuration?)
  - An entirely separate route?
  - How does this choice affect the previous point about swagger files?
- Where does the source live?
  - openshift/assisted-service? (Like controllers)
  - New repository in the OpenShift GitHub organization?
  - Elsewhere?
